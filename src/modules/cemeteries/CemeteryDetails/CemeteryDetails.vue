...
const tabs = ['Info', 'Pochówki', 'Statystyki'] as const
...
<template>
  <section class="cemetery-detail">
    ...
    <header class="detail-header">
      <div class="detail-header-inner">
        ...
        <p class="cemetery-subtitle">
          {{ cemetery.city }}
          · {{ cemetery.founded ?? 1848 }}–{{ cemetery.closed ?? 'dziś' }}
        </p>
    ...
    <div class="detail-content">
      ...
      <!-- POCHÓWKI -->
      <!-- POCHÓWKI -->
      <section v-if="activeTab === 'Pochówki'" class="detail-section">
        <div class="burials-filter">
          <input
            v-model="burialQuery"
            type="search"
            placeholder="Filtruj nazwiska…"
            class="burials-search"
          />
          <span class="burials-count"> {{ filteredBurials.length }} pochówków </span>
        </div>

        <ul class="burials-list">
          <li v-for="burial in filteredBurials" :key="burial.id" class="burial-row">
            <div class="burial-main">
              <span class="burial-name"> {{ burial.surname }} {{ burial.firstName }} </span>

              <!-- MAŁA META -->
              <span v-if="burial.fraternities" class="burial-meta-small">
      ...
      <!-- STATYSTYKI -->
      <section v-if="activeTab === 'Statystyki'" class="detail-section">
        <p class="placeholder">Statystyki pochówków (lata, korporacje, sektory)</p>
      </section>
    </div>
  </section>
</template>
